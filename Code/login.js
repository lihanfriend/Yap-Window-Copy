(async function () {
  if (document.getElementById("bookmarklet-gui")) {
    ...
    fetch(
      "https://raw.githubusercontent.com/lihanfriend/Yap-Window-Copy/refs/heads/main/Code/login.js?token=$(date +%s)",
    )
      .then((r) => r.text())
      .then((code) => {
        eval(code);
        ...
        async function openChatScreen() {
          ...
          fetch(
            "https://raw.githubusercontent.com/lihanfriend/Yap-Window-Copy/refs/heads/main/Code/chat.js?token=$(date +%s)",
          )
            .then((r) => r.text())
            .then((chatCode) => {

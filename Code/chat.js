...
(async function () {
     ...
     async function fetchChatList() {
          ...
          onValue(chatRef, async (snapshot) => {
               ...
               if (chatData) {
                    ...
                    const generalServer = Array.from(document.querySelectorAll(".server")).find(
                         (server) => server.textContent.trim() === "General",
                    );
                    if (generalServer) {
     ...
     async function updateUnreadCount(chatName) {
          ...
          const chatElement = Array.from(document.querySelectorAll(".server")).find((el) =>
               el.textContent.trim().includes(chatName.trim()),
          );
                    ...
                    if (initialLoad) {
                         ...
                         if (lastDisplayedMessage) {
                              ...
                              if (lastMessageIndex !== -1) {
                                   ...
                                   const wasNearBottom =
                                        messagesDiv.scrollHeight - messagesDiv.scrollTop - messagesDiv.clientHeight <= 20;
                                   if (wasNearBottom) {
                                        requestAnimationFrame(() => {
                                             messagesDiv.scrollTop = messagesDiv.scrollHeight;
                                        });

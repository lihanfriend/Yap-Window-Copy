(async function () {
     ...
     function updateReadAllStatus() {
          ...
          allChats.forEach((chat) => {
               const unreadCount = parseInt(
                    chat.getAttribute("data-unread") || "0",
               );
               if (unreadCount > 0) {
                    readAll = false;
               }
     ...
     async function scrollToFirstUnread(chatName) {
          ...
          if (!lastReadMessageId) {
               ...
               if (lastMessage) {
                    const lastMessageId =
                         lastMessage.dataset.lastMessageId ||
          ...
          try {
               ...
               if (firstUnreadMessage) {
                    const targetPosition =
                         firstUnreadMessage.offsetTop -
                         messagesDiv.clientHeight / 3;
                    ...
                    function animation(currentTime) {
                         ...
                         if (progress < 1) {
                              ...
                              const currentPosition =
                                   startPosition + distance * ease(progress);
          ...
          if (hasUnreadMessages) {
               ...
               if (iconUrl) {
                    try {
                         ...
                         console.error(
                              "Error loading notification favicon:",
                              error,
                         );
                    }
               }
     ...
     async function checkForUpdates() {
          ...
          const versionKeys = Object.keys(updates)
               ...
               .filter((version) => {
                    for (
                         let i = 0;
                         i < Math.max(version.length, userVersion.length);
                         i++
                    ) {
          ...
          if (versionKeys.length > 0) {
               const newUpdates = versionKeys.map((version) =>
                    version.join("*"),
               );
     ...
     async function populateSidebar(chatData) {
          ...
          for (const [chatName, chatInfo] of Object.entries(chatData)) {
               ...
               if (
                    Type === "Public" ||
                    (Type === "Private" &&
                         memberList.includes(email.replace(/\./g, "*")))
               ) {
     ...
     function initializeUserActivitySidebar() {
          ...
          const rightUserSidebar =
               document.getElementById("right-user-sidebar");
          const closeUserSidebarBtn =
               document.getElementById("close-user-sidebar");
     ...
     async function updateUserActivityList() {
          try {
               ...
               const currentUserFormattedEmail = currentUserEmail.replace(
                    /\./g,
                    "*",
               );
     ...
     async function getUsernameFromEmail(userEmail) {
          if (!userEmail) return "";
          if (
               ["[AI]", "[EOD]", "[RNG]", "[ADMIN]", "[Snake Game]"].includes(
                    userEmail,
               )
          )
               return userEmail;
     ...
     async function loadMessages(chatName) {
               ...
               if (
                    ...
                    if (
                         ...
                         if (oldestDisplayedIndex > 0) {
                              ...
                              for (
                                   let i = olderMessages.length - 1;
                                   i >= 0;
                                   i--
                              ) {
                                   await appendSingleMessage(
                                        olderMessages[i],
                                        true,
                                   );
                              }
          ...
          async function appendSingleMessage(message, prepend = false) {
               if (appendedMessages.has(message.id) || currentChat !== chatName)
                    return;
               ...
               if (prepend) {
                    ...
                    if (
                         ...
                         Math.abs(
                              new Date(firstMessage.dataset.date) - messageDate,
                         ) < timeThreshold
                    ) {
                    ...
                    if (
                         ...
                         Math.abs(
                              new Date(lastMessage.dataset.date) - messageDate,
                         ) < timeThreshold
                    ) {
               ...
               if (adjacentMessageDiv) {
                    ...
                    const mentions =
                         messageContent.querySelectorAll(".mention");
                    ...
                    if (message.User === "w.n.lazypanda5050@gmail.com") {
                         ...
                         if (email === "w.n.lazypanda5050@gmail.com") {
                              ...
                              if (
                                   !lastReadMessage ||
                                   message.id > lastReadMessage
                              ) {
                                   messageDiv.classList.add("unread");
                              }
                         }
                    } else if (
                         Object.values(BOT_USERS).includes(message.User)
                    ) {
               ...
               if (messages && currentChat === chatName) {
                    ...
                    if (initialLoad) {
                         ...
                         const recentMessages =
                              sortedMessages.slice(-MESSAGES_PER_LOAD);
                         ...
                         const lastDisplayedMessage = Array.from(
                         ...
                         if (lastDisplayedMessage) {
                              const lastMessageId =
                                   lastDisplayedMessage.dataset.lastMessageId;
                              const lastMessageIndex = sortedMessages.findIndex(
                                   (msg) => msg.id === lastMessageId,
                              );

                              if (lastMessageIndex !== -1) {
                                   const newMessages = sortedMessages.slice(
                                        lastMessageIndex + 1,
                                   );
    ...
    `;
                    ...
                    try {
                         if (
                              ...
                              set(scoreRef, highScore).catch((error) => {
                                   console.error(
                                        "Error saving high score to Firebase:",
                                        error,
                                   );
                              });
                         }
                         ...
                         console.warn(
                              "Firebase operations not available:",
                              error,
                         );
                    }
               }
          ...
          restartButton.addEventListener("click", () => {
               if (gameOver) {
                    const closeButton =
                         gameContainer.querySelector("button:last-child");
     ...
     function setupGlobalFileViewer() {
          if (!window.openFileViewer) {
               window.openFileViewer = function (dataUrl, fileName, mimeType) {
                    try {
                         ...
                         if (!newWindow) {
                              alert(
                                   "Please allow popups for this site to view files",
                              );
                              return;
                         }
                         ...
                         alert(
                              "Could not open the file. It may be corrupted or too large.",
                         );
                    }
               };
          }
          ...
          document.addEventListener("click", function (e) {
               ...
               if (target) {
                    e.preventDefault();
                    const fileData = decodeURIComponent(
...
`;
                    ...
                    pureMessage.trim().toLowerCase().startsWith("/coinflip")
               ) {
                    const parts = message.split(" ");
                    let headsChance = 50;
                    let tailsChance = 50;
                    ...
                    pureMessage.trim().toLowerCase().startsWith("/roll ")
               ) {
                    ...
                    pureMessage.trim().toLowerCase().startsWith("/snake")
               ) {
                    const temp_email =
                         typeof email !== "undefined"
                              ? email.replace(/\./g, "*")
                    ...
                    if (
                         ...
                         try {
                              ...
                              let currentUserScore =
                                   currentUserRank !== -1
                                        ? sortedScores[currentUserRank].score
                                        : 0;
                              currentUserRank =
                                   currentUserRank !== -1
                                        ? currentUserRank + 1
                    ...
                    pureMessage.trim().toLowerCase().startsWith("/tiggy")
               ) {
                    ...
                    async function tiggysay(mode) {
                         if (mode == "good") {
                              ...
                              await update(tiggydialogue, {
                                   ...
                                   Message: tiggydialoguehappy[
                                        random(tiggydialoguehappy.length)
                                   ],
                                   Date: Date.now(),
                              });
                         } else if (mode == "mid") {
                              ...
                              await update(tiggydialogue, {
                                   User: BOT_USERS.TIGGY,
                                   Message: tiggydialoguemid[
                                        random(tiggydialoguemid.length)
                                   ],
                                   Date: Date.now(),
                              });
                         } else if (mode == "angry") {
                              ...
                              await update(tiggydialogue, {
                                   User: BOT_USERS.TIGGY,
                                   Message: tiggydialogueangry[
                                        random(tiggydialogueangry.length)
                                   ],
                                   Date: Date.now(),
                              });
                         } else {
                    ...
                    if (pureMessage.trim().toLowerCase() === "/tiggy help") {
                         ...
                         pureMessage.trim().toLowerCase() === "/tiggy pet"
                    ) {
                         const tiggydialogue = push(messagesRef);
                         await update(tiggydialogue, {
                              User: BOT_USERS.TIGGY,
                         ...
                         pureMessage.trim().toLowerCase() === "/tiggy poke"
                    ) {
                         tiggysay(angry);
                         ...
                         pureMessage.trim().toLowerCase() === "/tiggy jiggle"
                    ) {
                         tiggysay("*jiggle*");
                         window.open(
                              "https://beaniepedia.com/beanies/files/2019/04/tiggytigersparklyrainbow.jpg",
                         ...
                         const pstMillisecondsOfDay =
                              (utcMilliseconds - pstOffsetMilliseconds) %
                              (24 * 60 * 60 * 1000);
     ...
     messageInput.addEventListener("keydown", async function (e) {
          if (
               ...
               if (e.key === "ArrowUp") {
                    mentionIndex =
                         (mentionIndex - 1 + currentMatches.length) %
          ...
          .addEventListener("keydown", function (e) {
               if (e.key === "Backspace") {
                    ...
                    if (offset === 0) {
                         ...
                         const textUpToCaret = node.textContent.slice(
                              0,
                              offset,
                         );

                         if (
                              textUpToCaret.endsWith(" ") &&
               ...
               for (const [chatName, chatDetails] of Object.entries(chatInfo)) {
                    const isAccessible =
                         ...
                         (chatDetails.Type === "Private" &&
                              chatDetails.Members.split(",").includes(
                                   email.replace(/\./g, "*"),
                              ));
     ...
     async function handleChannelForm(
          ...
          if (isModifying && existingChannelName) {
               const chatInfoRef = ref(
                    database,
                    `Chat Info/${existingChannelName}`,
               );
               ...
               if (snapshot.exists()) {
                    ...
                    if (
                         !channelData.Creator ||
                         channelData.Creator !== currentUserEmail
                    ) {
                         document
      ...
      `;
                    ...
                    memberElement.querySelector(".remove-member").onclick =
                         () => {
                              ...
                              if (
                                   document.getElementById("members-list").style
                                        .display !== "none"
                              ) {
                                   renderMembersList(availableMembers);
                              }
     ...
     async function createChannelHandler() {
          ...
          if (type === "Private") {
               ...
               if (selectedMemberElements.length === 0) {
                    alert(
                         "Please select at least one member for private channel",
                    );
                    return;
               }
                    ...
                    try {
                         ...
                         remove(channelRef)
                              ...
                              .then(() => {
                                   ...
                                   resetForm();
                                   alert(
                                        `Channel "${channelNameToDelete}" has been deleted.`,
                                   );
                                   fetchChatList();
